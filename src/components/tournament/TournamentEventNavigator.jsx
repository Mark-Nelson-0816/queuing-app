...
export default function TournamentEventNavigator({
  ...
  return (
    <aside className="self-start min-w-0 overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] 2xl:sticky 2xl:top-0">
      <div className="border-b border-[var(--border)] p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            type="button"
            onClick={onCreate}
            className="inline-flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-xl bg-[var(--primary)] px-3 py-2 text-xs font-semibold text-white hover:bg-[var(--primary-hover)]"
          >
          ...
          return (
            ...
            >
              <div className="flex min-w-0 items-start justify-between gap-2">
                ...
                <span className={`shrink-0 whitespace-nowrap rounded-full px-2 py-0.5 text-[10px] font-semibold capitalize ${getTournamentStatusClasses(tournament.status)}`}>
                  {tournament.status}
                </span>
              </div>
              <div className="mt-2 flex min-w-0 items-center gap-1.5 text-xs text-[var(--text)]">
                <CalendarDays className="h-3.5 w-3.5" />
                <span className="truncate">{formatTournamentDate(tournament.startDate)} - {formatTournamentDate(tournament.endDate)}</span>
              </div>
